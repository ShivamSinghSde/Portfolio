...
const Navbar = () => {
    ...
    return (
        <nav
            className={` top-0 w-full z-50 transition-duration-300 px-[7vw] md:px-[7vw] lg:px-[20vw] ${isScrolled
            ...
            <div className="fixed text-white w-full px-5 flex justify-between items-center">
                ...
                <ul className="hidden md:flex gap-6 text-gray-300 cursor-pointer">
                    {menu.map((item) => (
                        <li
                            key={item.id}
                            className={`cursor-pointer  hover:text-[#f1bc71] ${activeSection === item.id ? "text-[#f1bc71]" : ""
                                }`}
                        >
